...
class SparseAttentionLayerBlock(nn.Module):
    def __init__(self, dim, restormer_num_heads=3, restormer_ffn_expansion_factor=2., tlc_flag=True, tlc_kernel=48,
...
class AGDN(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=24, num_block=7, num_out_ch=3, upscale=4,
    ...
    def forward(self, input):
        ...
        out = self.upsampler(
            self.c2(self.GELU(self.c1(torch.cat([out_B1, out_B2, out_B3, out_B4, out_B5, out_B6, out_B7], dim=1)))) + out_fea) + self.mean
...
if __name__ == '__main__':
    ...
    model = AGDN(num_feat=42, upscale=2)
